	...
	while (true) {
		...
		for (int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if (FD_ISSET(s, &readfds))
			{
				...
				//если клиент отправляет первое сообщение - это его ник
				if (names.size() <= i)
				{
				...
				else if (colors.size() <= i && names.size() > i)  //если второе - это цвет уведомлений
				{
					char client_color[DEFAULT_BUFLEN];
					int client_message_length = recv(s, client_color, DEFAULT_BUFLEN, 0);
					client_color[client_message_length] = '\0';
					colors.push_back(client_color);

					//сообщаем клиентов о новом клиенте
					string new_client = "4:Client " + names[i] + " is added.";
					for (int i = 0; i < MAX_CLIENTS; i++) {
						if (client_socket[i] != 0) {
				...
				else  //остальное - обычное уведомление
				{
					...
					if (check_exit == "off")
					{
						string new_client = "4:Client " + names[i] + " is off.";
						for (int i = 0; i < MAX_CLIENTS; i++) {
							if (client_socket[i] != 0) {
								send(client_socket[i], new_client.c_str(), new_client.size(), 0);
							}
						}
						cout << "Client " << names[i] << " is off.\n";
						client_socket[i] = 0;
					}
					else
					{
						string message = colors[i] + ":" + names[i] + ": " + client_message;
						char mess[DEFAULT_BUFLEN];
						strcpy(mess, message.c_str());

						string temp = mess;      //запоминаем историю
						temp += "\n";
						history.push_back(temp);

						for (int i = 0; i < MAX_CLIENTS; i++) {
							if (client_socket[i] != 0) {
								send(client_socket[i], mess, strlen(mess), 0);
							}
						}
					}
				}
