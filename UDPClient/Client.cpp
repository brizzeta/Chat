...
DWORD WINAPI Receiver(void* param)
{
    while (true) {
        ...
        char num[2];
        int i = 0;
        while (response[i] != ':')  //узнать цвет сообщения клиента
        {
            num[i] = response[i];
            i++;
        }

        if (atoi(num) >= 0 && atoi(num) <= 15)  //выставляем цвет сообщения
        {
            SetConsoleTextAttribute(h, atoi(num));
        }
...
int main()
{
    ...
    char color[DEFAULT_BUFLEN];
    cin.getline(color, DEFAULT_BUFLEN);
    send(client_socket, color, strlen(color), 0);
